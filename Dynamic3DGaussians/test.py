...
from dataset_utils.dataset_readers import setup_camera
from helpers import quat_mult#, setup_camera
...
def load_scene_data(seq, exp, masking=False, masking_method="ste", prune=False, pruning_method="avg", seg_as_col=False, threshold=0.01):
    #exp = "discussion-training-day"
    #exp = "1_experiment_panoptic"
    #exp = "discussion-training-day-3000-50"
    #exp = "panoptic-original-dis"
    #exp = "exp12"
    #exp = "exp14-discussion"
    #exp = 'exp0406-disc-loss/2*1716-full'
    exp = "threshold-0.0010-every-500-iter"
...
def test(seq, exp, masking=False, masking_method="ste", prune=False, pruning_method="avg", threshold=0.01, save_prefix=""):
    ...
    md = json.load(open(f"videos-named/{seq}/test_meta.json", 'r'))  # metadata
...
#         test(sequence, exp_name, masking=True, masking_method="sigmoid", prune=False, threshold=0.01, save_prefix="mask_sigmoid_0.01")
    ...
    # ------------------------- MINE --------------------------
    #exp_name = "discussion-test-1"
    #for sequence in [""]:
    #    test(sequence, exp_name, masking=False, prune=False)

    #exp_name = "exp12-disc-test"
    #for sequence in ["basketball"]:
    #    test(sequence, exp_name, masking=False, prune=False)

    exp_name = "threshold-0.0010-every-500-iter"
    for sequence in [""]:
        test(sequence, exp_name, masking=False, prune=False, save_prefix="threshold-test")